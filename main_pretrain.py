...
from data.download import download_imagenet
...
parser.add_argument("--download_imagenet", action="store_true", help="Download the ImageNet dataset if not already present.")
...
# Download ImageNet dataset if not already present
if args.download_imagenet and not os.path.exists('./data/imagenet'):
    print("Downloading ImageNet dataset (this may take a while)...")
    download_imagenet(val_only=val_set)
